...
func Mount(ntype, addr, aname string, msize uint32, user p.User) (*Clnt, error) {
...
func MountConn(c net.Conn, aname string, msize uint32, user p.User) (*Clnt, error) {
	clnt, err := Connect(c, msize+p.IOHDRSZ, true)
	if err != nil {
		return nil, err
	}
